...
class UserStatsService {
  ...
  static Future<void> addCustomQuests(List<QuestModel> quests) async {
    if (useMock) {
      _mockQuests.addAll(quests);
      return;
    }
    
    // UUID will be generated by Supabase if we don't send id, but we already have id from AiQuestService. 
    // However, if table expects gen_random_uuid(), it's better to exclude id for inserts if it causes issues, 
    // but QuestModel's toJson includes id. Let's see QuestModel toJson in category_models.dart.
    // QuestModel toJson includes 'id': id. So we should insert it directly.
    final questsJson = quests.map((q) {
      final json = q.toJson();
      json.remove('id'); // Let Supabase handle the ID generation to avoid conflicts, or use the generated UUID. We'll use the UUID from AI service, but Supabase might require id to be excluded. Let's exclude it to be safe.
      return json;
    }).toList();
    
    await _client.from('quests').insert(questsJson);
  }
  ...
  static Future<void> addCustomFinancialTodos(List<FinancialTodo> todos) async {
    if (useMock) return;
    ...
    await _client.from('financial_todos').insert(jsonList);
  }
}
